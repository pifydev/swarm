...
test("queued and running are different facts and read differently", () => {
  const queued = pendingResult({ id: "a", kind: "queued", startedAt: 0, now: 0, collectWith: "swarm_status" });
  assert.match(queued.text, /queued behind the concurrency cap/);
  assert.equal(queued.details.status, "queued");

  const running = pendingResult({ id: "a", kind: "running", startedAt: 0, now: 0, collectWith: "swarm_status" });
